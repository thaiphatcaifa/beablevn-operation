...
const StaffLayout = () => {
  ...
  // --- LOGIC XỬ LÝ VÀ LƯU THÔNG BÁO (CẬP NHẬT) ---
  useEffect(() => {
    if (user) {
      // 1. Xin quyền, lấy FCM Token và lưu vào database
      requestForToken().then((token) => {
        // Kiểm tra token có hợp lệ và hàm updateStaffInfo tồn tại
        if (token && typeof updateStaffInfo === 'function') {
          updateStaffInfo(user.id, { fcmToken: token });
        }
      }).catch(err => console.error('Lỗi lấy token:', err));

      // 2. Lắng nghe thông báo tới khi đang mở app (Foreground)
      const listenForMessages = async () => {
        try {
          const payload = await onMessageListener();
          console.log('Nhận được thông báo:', payload);
          
          // Hiển thị thông báo trên trình duyệt (Native browser notification)
  ...
  }, [user]); // Bỏ updateStaffInfo khỏi mảng dependency tránh loop nếu nó không được wrap trong useCallback
  // ----------------------------------------------
        ...
        <div style={{ background: 'white', padding: '24px', borderRadius: '16px', marginBottom: '24px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #f3f4f6' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '0.75rem', color: '#6b7280', textTransform: 'uppercase', marginBottom: '4px', fontWeight: '600' }}>
                        UBI ({((Number(currentUserInfo.ubi1Percent) + Number(currentUserInfo.ubi2Percent)) / 2) || 100}%)
                    </div>
                    <strong style={{ color: '#059669', fontSize: '1.5rem', fontWeight: '700', letterSpacing: '-0.025em' }}>
                        ...
                        <span style={{fontSize: '0.875rem', color: '#6b7280', fontWeight: '500', marginLeft: '3px'}}>đ</span>
                    </strong>
                </div>
            </div>
        </div>
